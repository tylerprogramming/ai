...
from CustomFileWriterTool import CustomFileWriterTool
...
class NewsAggregationCrew():
    ...
    @agent
    def ai_news_file_writer(self) -> Agent:
        return Agent(
            config=self.agents_config['ai_news_file_writer'],
    ...
    @task
    def ai_news_file_writing_task(self) -> Task:
        return Task(
            config=self.tasks_config['ai_news_file_writing_task'],
            agent=self.ai_news_file_writer(),
            # output_json=FileWriterOutput,
        )
